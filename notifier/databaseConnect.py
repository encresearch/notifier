...
class Database():
	...
	def getEmails(self, inspectorJSONPackage=None):
	
		if inspectorJSONPackage == None:
			#Query to get all unique emails from the database
			get_email_query = "SELECT DISTINCT email FROM emails"
		else:
			get_email_query = self.makeEmailQueryBasedOnTopic(inspectorJSONPackage)
	...
	def makeEmailQueryBasedOnTopic(self, inspectorJSONPackage):
		
		get_email_query_1 = "SELECT DISTINCT email FROM emails"
		get_email_query_2 = get_email_query_1 + " WHERE " + inspectorJSONPackage['topic'] + " = TRUE"
